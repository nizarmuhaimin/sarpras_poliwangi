<!DOCTYPE html>
<html lang="en">

<head>

    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="">
    <meta name="author" content="">

    <title>Sarana & Prasarana - Masuk</title>

    <!-- Custom fonts for this template-->
    <link href="{{asset ('backend/vendor/fontawesome-free/css/all.min.css')}}" rel="stylesheet" type="text/css">
    <link
        href="https://fonts.googleapis.com/css?family=Nunito:200,200i,300,300i,400,400i,600,600i,700,700i,800,800i,900,900i"
        rel="stylesheet">

    <!-- Custom styles for this template-->
    <link href="{{asset ('backend/css/sb-admin-2.min.css')}}" rel="stylesheet">
    

</head>

<body style="background-color:rgb(00, 48, 78)">
    <div style="
                width: 100px;
                height: 100px;
                stop: 100px;
                left: 100px;
                overflow: hidden;"></div>
    <div class="text-center">
        <img src="{{asset('img/logo-poliwangi.png')}}"  alt="Deskripsi Gambar" width="180" >
        <div style="
                width: 25px;
                height: 25px;
                stop: 25px;
                left: 25px;
                overflow: hidden;"></div>
        <div class="text-center"
        style="
        /* width: 578px; */
        color: rgba(255,255,255,1);
        top: 404px;
        /* left: 671px; */
        
        font-weight: ExtraBold;
        font-size: 20px;">
        SARANA DAN PRASARANA</div>
        <div style="
        /* width: 578px; */
        color: rgba(255,255,255,1);
        top: 404px;
        /* left: 671px; */
        /* bottom: 20px; */
    
        font-weight: ExtraBold;
        font-size: 20px;">
        POLITEKNIK NEGERI BANYUWANGI</div>
    </div>
    <div class="container">
        <!-- Outer Row -->
        <div class="row justify-content-center">
            <div class="col-xl-5 col-lg-6 col-md-3">
                <div class="card o-hidden border-0 shadow-lg my-5">
                    <div class="card-body p-0">
                        <!-- Nested Row within Card Body -->
                        <div class="row">
                            
                            <div class="col-lg-12">
                                <div class="p-5">
                                    {{-- <div class="text-center">
                                        <img src="{{asset('img/logo-poliwangi.png')}}"  alt="Deskripsi Gambar" width="160" >
                                        <h1 class="h4 text-gray-900 mb-4">Welcome Back Draivi!</h1>
                                    </div> --}}
                                    
                                    <form action="{{ route('login') }}" method="post" class="user">
                                            @csrf
                                        <div style="
                                        font-weight: Medium;" class="form-group">
                                            <label for="exampleFormControlInput1" class="form-label">Username</label>
                                            <input type="email" class="form-control form-control-user" name="email"
                                                class="form-label"id="exampleInputEmail" aria-describedby="emailHelp"
                                                placeholder="Enter Email Address/Username..." />
                                        </div>
                                        <div style="
                                        font-weight: Medium;" class="form-group">
                                            <label for="exampleFormControlInput1" class="form-label">Password</label>
                                            <input type="password" class="form-control form-control-user" name="password"
                                                id="exampleInputPassword" placeholder="Password" />
                                        </div>
                                        <div style="
                                        font-weight: Medium;
                                        font-size: 15px;" class="form-group">
                                            <div class="custom-control custom-checkbox small">
                                                <input type="checkbox" class="custom-control-input" id="customCheck">
                                                <label class="custom-control-label" for="customCheck">Ingat Saya</label>
                                                
                                            </div>
                                           
                                        </div>
                                        <!-- <a href="{{ ('admin.admin_master') }}" class="btn btn-dark btn-user btn-block"> -->
                                            <!-- Login
                                        </a> -->
                                        <hr>
                                        <!-- <a href="index.html" class="btn btn-google btn-user btn-block">
                                            <i class="fab fa-google fa-fw"></i> Login with Google
                                        </a> -->
                                        <!-- <a href="index.html" class="btn btn-facebook btn-user btn-block">
                                            <i class="fab fa-facebook-f fa-fw"></i> Login with Facebook
                                        </a> -->
                                        <button type="submit" class="btn btn-dark btn-user btn-block" href="{{ ('auth.login') }}">Masuk</button>
                                    </form>
                                    <hr>
                                    <div style="
                                    font-weight: Medium;
                                    font-size: 15px;" class="text-center">
                                        <a >Belum Memiliki Akun?</a><a style="color:red"class="small" href="{{ url('/register') }}"> Daftar</a>
                                            <div style="
                                            font-weight: Medium;
                                            font-size: 15px;" class="text-center">
                                        <a href="{{ ('auth.forgot_password') }}">Lupa Password?</a>
                                            </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

            </div>

        </div>

    </div>

    <!-- Bootstrap core JavaScript-->
    <script src="{{asset ('backend/vendor/jquery/jquery.min.js')}}"></script>
    <script src="{{asset ('backend/vendor/bootstrap/js/bootstrap.bundle.min.js')}}"></script>

    <!-- Core plugin JavaScript-->
    <script src="{{asset ('backend/vendor/jquery-easing/jquery.easing.min.js')}}"></script>

    <!-- Custom scripts for all pages-->
    <script src="{{asset ('backend/js/sb-admin-2.min.js')}}"></script>

</body>

</html>